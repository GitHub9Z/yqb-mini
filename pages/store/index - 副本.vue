														...
														<view class="m-text">
															...
															<view class="m-distance" >
																...
																<block v-else>
																	...
																	<hx-number-box @addChange="goodsAdd" @lessChange="goodsLess" :value="rowData.number" :rowData="rowData" :clickTime="animaTime" @addChangeTwo="touchOnAddGoods('.addEle_' + i,rowData)"></hx-number-box>
																</block>
					...
					<view class="" style="flex: 1;">
						<scroll-view scroll-y="true" class="goods-list-scroll" :scroll-top="carGoodsScrollTop">
							<view class="goods-list">
								<view class="box" v-for="(rowData,i) in shoppCart" :key="rowData.id + '_' + (rowData.current_form ? rowData.current_form.id : '') " v-if="rowData.number>0">
									...
									<view class="m-store-item">
										...
										<view class="m-text">
											...
											<view :class="'addEle2_' + rowData.id" class="jumpPosition">
												...
												<hx-number-box  @addChangeTwo="touchOnAddGoods('.addEle2_' + rowData.id,rowData)" @addChange="goodsAdd" @lessChange="goodsLess" :value="rowData.number" :rowData="rowData" :clickTime="animaTime"></hx-number-box>
					...
					<view class="bottom">
						...
						<view class="form-btn-box">
							...
							<block v-else>
								...
								<hx-number-box @change="formAddGoodsChange" @addChange="formAddGoodsChange" @lessChange="formLessGoodsChange" :value="currentGoodsData.number" :rowData="currentGoodsData" :clickTime="animaTime + 200" @addChangeTwo="touchOnAddGoods('.addEle_gg',currentGoodsData)"></hx-number-box>
	...
	export default {
		...
		methods: {
			...
			formAddGoodsChange(number,goodsData){
				...
				that.goodsAdd(number,that.currentGoodsData)
			},
			//减商品购物车
			formLessGoodsChange(number,goodsData){
				let that = this;
				if(number >= 0){
					for(let i in goodsData.form_child){
						if(goodsData.form_child[i].select == true ){
				...
				that.goodsLess(number,that.currentGoodsData)
			},
			//选择规格
			...
			touchOnAddGoods(ele,data){
				...
				//this.goodsAdd(data.number,data)
			},
			goodsAdd(number,rowData){
				var that = this
				...
				// 获取全部门店购物车
				let cartsAll = uni.getStorageSync(that.shoppingCartStorageName) || [];
				// 多规格商品
				...
				if(cartsAll.length != 0){
					isFirstAddGoods = false
					//根据缓存数据 获取该商户的购物车
					for(let i in cartsAll){
						if(cartsAll[i].store_id == that.storeData.store_id){
							shoppCart = cartsAll[i].shopping_cart ? cartsAll[i].shopping_cart : [];
							break;
						}
					}
					//检查该商品是否为第一次添加，
					for(var i in shoppCart){
						if(shoppCart[i].id == rowData.id){
							// 商品没有规格，直接跳出，并且该商品非第一次添加
							// 有规格的商品 比较规格是否也相同
							if(!rowData.current_form || shoppCart[i].current_form.id == rowData.current_form.id){
				...
				// 用户第一次添加商品,需将门店信息加入
				if(isFirstAddGoods){
					cartsAll.push(that.storeData)
				}
				console.log(number,shoppCart)
				//购物车商品数据缓存至本地
				uni.setStorageSync(that.shoppingCartStorageName,cartsAll);
			},
			goodsLess(number,rowData){
				var that = this
				number = Number(number)
				// 该商户购物车
				let shoppCart = [];
				...
				cartsAll = JSON.parse(JSON.stringify(cartsAll))
				// 获取该门店的购物车
				for(let i in cartsAll){
					if(cartsAll[i].store_id == that.storeData.store_id){
						shoppCart = cartsAll[i].shopping_cart ? cartsAll[i].shopping_cart : [];
				...
				for(var i in shoppCart){
					if(shoppCart[i].id == rowData.id){
						...
						// 有规格的商品 比较规格是否也相同
						let zd = rowData.current_form ? (shoppCart[i].current_form.id == rowData.current_form.id ? true : false) : true
						if(zd){
							if(number <= 0){
								deleteGoods = shoppCart[i];
							}else{
				...
				for(let i in cartsAll){
					if(cartsAll[i].store_id == that.storeData.store_id){
						cartsAll[i].shopping_cart = shoppCart
						break;
					}
				}
				//购物车商品数据缓存至本地
