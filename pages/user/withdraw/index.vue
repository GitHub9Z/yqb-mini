		...
		<view class="withdraw">
			<view class="withdraw-head">
				...
				<view class="withdraw-head-way">
					<view class="withdraw-head-way-1">
						<image class="way-icon" src="/static/img/money.png"></image>
			...
			<view :class="['keyboard', keyboradShow ? '' : 'active', isIphoneX ? 'isIphone' : '']">
				...
				<view class="keyboard-item delte" @tap="keyboradDel()">
					<image class="img" src="/static/img/del.png" mode="aspectFill" :lazy-load="true"></image>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				this.lowest = 1;
				this.fee = 0.5;
				console.log('余额', this.get_user_info.wallet)
				this.pool = this.get_user_info.wallet;
				this.$nextTick(() => {
			...
			handleShowModel() {
				...
				uni.showModal({
					title: '提示',
					content: `提现金额：${this.money}.00 元\n实际到账：${this.real_money} 元\n服务费用：${this.service_fee} 元\n服务费率：${this.fee}%
					`,
					confirmColor: '#e54d42',
					confirmText: '提现',
					success: (res) => {
			...
			getMoney() {
				uni.request({
					url: 'https://www.imgker.com/yqb/history/withdraw', //仅为示例，并非真实接口地址。
					data: {
						sum: this.money
					},
					header: {},
